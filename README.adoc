...
----
With the `/version` endpoint corrected to return a [hotspot=decimal file=0]`decimal type`, press the `enter/return` key to re-run the tests in the command-line session where the `system` microservice was started.
