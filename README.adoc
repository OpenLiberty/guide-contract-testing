...
The [hotspot=builder file=0]`createPactEncoding()` method defines the minimal expected response for a specific endpoint and is also known as an interaction.
...
When integrating the pact framework in a CI/CD build pipeline, the `mvn verify` goal can be used instead to generate the pact file.
...
In the command-line session where the `system` microservice was started, press the `enter/return` key to run the tests to verify the pact file. When integrating the pact framework in a CI/CD build pipeline, the `mvn test` goal can be used instead to verify the pact file from the pact broker.
