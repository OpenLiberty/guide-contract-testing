...
The `InventoryPactIT` class contains a [hotspot=mockprovider file=0]`PactProviderRule`
...
The [hotspot=builder file=0]`createPactServer()` method defines the minimal expected responsezfor a specific endpoint, which is known as an interaction.
