...
Correct the [hotspot=decimal file=0]`system.properties.version` value to decimal and press the `enter/return` key to re-run the tests in the command-line session where the `system` microservice was started.
[role="code_command hotspot file=0", subs="quotes"]
----
